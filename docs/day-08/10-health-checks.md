...
[← Client-Side vs Server-Side Discovery](./09-client-side-vs-server-side-discovery.md) | [Day 8 Index](./README.md) | [Next: Service Mesh Introduction →](./11-service-mesh-introduction.md)
...
See [Day 7: Redundancy and HA](../day-07/04-redundancy-and-high-availability.md).
...
Expose dedicated health URLs and probe them continuously.

```
GET /health/live   → process up?
GET /health/ready  → safe to receive traffic?
GET /health/startup → finished init? (optional)
```

| Consumer | Uses |
|----------|------|
| Kubernetes | liveness / readiness / startup probes |
| Load balancer | Remove bad targets |
| Gateway | Route around failing upstreams |
| Monitoring | Alert on failing checks |

Load balancers, orchestrators, and gateways all depend on these endpoints staying honest.
