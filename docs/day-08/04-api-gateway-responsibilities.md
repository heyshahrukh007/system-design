...
[← Why API Gateway](./03-why-api-gateway.md) | [Day 8 Index](./README.md) | [Next: Routing and Load Balancing →](./05-routing-and-load-balancing.md)
...
| Pattern | Meaning |
|---------|---------|
| **Routing** | Path/host/header → correct service |
| **Offloading** | TLS, auth, compression, rate limits at the edge so services stay thin |
| **Aggregation** | One client response built from several service calls |

```
Offloading:   Client ──TLS+JWT+limit──▶ Gateway ──▶ Services
Aggregation:  GET /dashboard → user + orders + notifs
Routing:      /orders/* → order-svc ; /pay/* → payment-svc
```

If aggregation grows complex, put it in a BFF and keep the shared gateway on routing + offloading.
