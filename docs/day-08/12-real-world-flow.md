...
[← Service Mesh Introduction](./11-service-mesh-introduction.md) | [Day 8 Index](./README.md)
...
User sees order placed; email delayed ([Day 6](../day-06/10-retry-dlq-and-idempotency.md))
```
...
| Async | Kafka queue | [Day 6](../day-06/README.md) |
| Failures | Circuit breaker, retry | [Day 7](../day-07/README.md) |

---

## External Configuration Store

Keep config **out of the image** — feature flags, connection strings, and limits live in a central store services read at runtime (or on refresh).

```
Services ──▶ Config store (etcd, Consul, AWS AppConfig, Spring Cloud Config)
                │
                └── versioned keys: /checkout/payment_timeout_ms = 2000
```

| Benefit | Detail |
|---------|--------|
| Change without redeploy | Tune timeouts, flags |
| One source of truth | All instances see same config |
| Audit / rollback | Version history |

Secrets stay in a **secret manager** (Vault, KMS) — not plain files in git.
