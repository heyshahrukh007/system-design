...
[← What Is a Message Queue](./02-what-is-a-message-queue.md) | [Day 6 Index](./README.md) | [Next: Queue, Pub/Sub, Streams →](./04-queue-pubsub-and-streams.md)
...
Sometimes you need async with a response later (**async request-reply**).
...
| Pattern | Flow |
|---------|------|
| **Reply queue** | Correlation ID; caller waits on reply topic |
| **Status resource** | `POST /exports` → `202` + `export_id`; client polls `GET /exports/{id}` |
| **Callback / webhook** | Worker POSTs result to caller URL |
| **Store + notify** | Write to S3/DB; push/email when ready |

Prefer **status API** for user-facing long jobs (minutes). Prefer **sync RPC** when the answer is needed in &lt; ~100–200 ms.

See also [queue patterns](./11-queue-patterns.md).
