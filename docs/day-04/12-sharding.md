...
[← Replication](./11-replication.md) | [Day 4 Index](./README.md)
...
## Federation (Functional Split)

**Federation** splits databases by **function / domain**, not by hashing the same table across shards.

```
Sharding:     users 0–N split across Shard0…Shard3 (same schema)

Federation:   Users DB  |  Orders DB  |  Inventory DB
              (different schemas, different owners)
```

| | Federation | Sharding |
|--|------------|----------|
| Split by | Bounded context / service | Rows of one dataset |
| Schema | Often different per DB | Same schema, many nodes |
| Cross-query | App joins or APIs | Scatter-gather / router |
| Fits | Microservices “DB per service” | Single huge table growth |

Federation and sharding can combine: an Orders service federated from Users, with Orders itself sharded by `user_id`.

Prefer federation when **teams and domains** diverge; prefer sharding when **one table** outgrows one machine.
