...
[← Normalization](./03-normalization.md) | [Day 4 Index](./README.md) | [Next: Joins / Lookups →](./05-joins-and-lookups.md)
...
When the primary store can’t answer a query efficiently (wrong key, document DB, or sharded data), keep a separate **index table** optimized for that lookup.

```
Primary:  orders (keyed by order_id)
Index:    orders_by_user (user_id → [order_ids])   updated on write
Query:    "orders for user 42" → index table → fetch orders by id
```

| Use when | Example |
|----------|---------|
| Access pattern ≠ primary key | Lookup by email, SKU, external_id |
| CQRS read model | Denormalized “list” table ([Day 12](../day-12/02-cqrs.md)) |
| Cross-shard discovery | Global unique email → shard map |

Keep the index table **updated in the same transaction** as the write when possible, or via outbox/CDC if it lives elsewhere.
